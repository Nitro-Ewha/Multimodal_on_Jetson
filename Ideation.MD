...
- 따라서 우리는 모델 경량화 및 추론 최적화와 같은 SW Optimization과, 프로파일링을 통해 멀티모달 연산의 병목을 분석하여 최적화된 하드웨어 아키텍처를 제안하는 HW Optimization과 같이 여러 접근 방법을 Jetson Nano에 적용시켜봄으로써, **온디바이스 멀티모달 대형 모델**의 퍼포먼스를 발전시키고자 한다. 
